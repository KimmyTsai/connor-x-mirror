-- Corner.X 城市反射鏡數據網 ── BigQuery schema
...
CREATE TABLE IF NOT EXISTS mirror_eye.intersections (
  ...
  road_width_source     STRING,      -- survey | osm | ai_estimated | NULL(無資料)
  corner_angle_deg  FLOAT64,         -- 轉角夾角，越接近90度視線越差
  sight_distance_m  FLOAT64,         -- 轉角視距，由 GIS 幾何推算
  sight_distance_source STRING,      -- survey | osm | ai_estimated | NULL(無資料)
  near_school       BOOL,
  near_market       BOOL,
  updated_at        TIMESTAMP
);

-- 既有專案的 intersections 表可能已建立過（沒有上面兩個 source 欄位），
-- 補跑這兩行讓舊表升級；新建的表已經含這兩欄，這兩行會直接跳過不報錯。
ALTER TABLE mirror_eye.intersections
  ADD COLUMN IF NOT EXISTS road_width_source STRING;
ALTER TABLE mirror_eye.intersections
  ADD COLUMN IF NOT EXISTS sight_distance_source STRING;
...
SELECT
  ...
  i.sight_distance_m,
  i.sight_distance_source,   -- survey | osm | ai_estimated | NULL
  i.road_width_m,
  i.road_width_source,       -- survey | osm | ai_estimated | NULL
...
SELECT
  ...
  r.road_names,
  r.accident_count,
  r.sight_distance_m, r.sight_distance_source,
  r.road_width_m, r.road_width_source,
